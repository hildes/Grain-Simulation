...
void GrainLJ::ajouteForce(Grain* grain)
{
    ...
    force += forceLJ(1.09 + (position_.norme() - rayon - grain->getRayon())/sigma) * position_.normalise();
 ...
 void GrainLJ:: ajouteForce(Obstacle* obstacle)
    {
        ...
        force += (2/sigma)*24*epsilon*(*this).forceLJ(1.09 + (distance - rayon)/sigma)*direction.normalise();
